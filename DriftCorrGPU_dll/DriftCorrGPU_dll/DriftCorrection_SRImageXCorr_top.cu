...
void SRDriftCorrData_TypeDef_GPU::CorrectSampleShift(string FileName, string oFileName,float PixelSize, int CorrFrameNum, cudaStream_t cstream)
{
...
void SRDriftCorrData_TypeDef_GPU::ResetFillImage(float *d_SRIntensityImg, int SRImageWidth, int SRImageHigh, cudaStream_t cstream)
{
	
	cudaMemsetAsync(d_SRIntensityImg, 0, SRImageWidth*SRImageHigh*sizeof(float));
	cudaStreamSynchronize(cstream);

}


void SRDriftCorrData_TypeDef_GPU::GetSliceShift(float *ShiftX, float *ShiftY, int CorrShiftBiasX, int CorrShiftBiasY, cudaStream_t cstream)
{
