...
void SRDriftCorrData_TypeDef_GPU::ShiftInterpolation()
{
	// set shift to 0
...
void SRDriftCorrData_TypeDef_GPU::ApplyShiftTop(string iFileName, string oFileName, int ShiftCorrEnable, cudaStream_t cstream)
{
...
void SRDriftCorrData_TypeDef_GPU::RenderSlice(string FileName, int RendGroup, float PixelSize, cudaStream_t cstream)
{
...
//	printf("cur rend:%d-%d\n", FirstFluoPos, EndFluoPos);
	...
	for (cnt = 0; cnt < ProcNum; cnt++)
	{
		...
		ImageRender(h_LocArry, d_LocArry, d_FillImage, PixelSize, SRShiftCorr_PixelZoom, SRImageWidth, SRImageHigh, CurFluoNum, cstream);
...
void SRDriftCorrData_TypeDef_GPU::CalcGroupNum( int TotalFrame, int CorrFrameNum)
{
...
void SRDriftCorrData_TypeDef_GPU::GetCorrGroupFluoPos(string FileName)
{
	int cnt = 0;
...
int SRDriftCorrData_TypeDef_GPU::GetAFrameEndFluoPos(string FileName, int OffsetFluoNum, int FindFrame)
{
...
void SRDriftCorrData_TypeDef_GPU::ImageRender(float *h_LocArry, float *d_LocArry, float *d_SRIntensityImg,  float PixelSize, float PixelZoom, int SRImageWidth, int SRImageHigh, int FluoNum, cudaStream_t cstream)
{
	...
	PointFillRegionWithPrec1 << <BlockNum, BlockDim, 0, cstream >> >(d_LocArry, d_SRIntensityImg, PixelSize, PixelZoom, SRImageWidth, SRImageHigh, FluoNum);

	cudaStreamSynchronize(cstream);
}

void SRDriftCorrData_TypeDef_GPU::gpuApplyShift(float *h_LocArry, int ShiftCorrEnable, int FluoNum, cudaStream_t cstream)
{
	int BlockDim = ThreadsPerBlock;
	int BlockNum = ((FluoNum + ThreadsPerBlock - 1) / ThreadsPerBlock);
...
double SRDriftCorrData_TypeDef_GPU::CrossCorrelation(int ShiftX, int ShiftY, int CorrShiftBiasX, int CorrShiftBiasY, cudaStream_t cstream)
{
	double TotalSum = 0;
...
int SRDriftCorrData_TypeDef_GPU::GetTotalFrame(string FileName)
{
...
void SRDriftCorrData_TypeDef_GPU::GetMaxImgSize(string FileName, int *ImageWidth, int *ImageHigh)
{
	...
	*ImageHigh = 0;
	...
	for (cnt = 0; cnt < ProcNum; cnt++)
	{
		...
		GetMaxImgSizeFromLocArry(h_LocArry, d_LocArry, &FindImgWidth, &FindImgHigh, CurFluoNum, cstream);
...
void SRDriftCorrData_TypeDef_GPU::GetMaxImgSizeFromLocArry(float* h_LocArry, float* d_LocArry, int* MaxImgWidth, int* MaxImgHigh, int FluoNum, cudaStream_t cstream)
{
