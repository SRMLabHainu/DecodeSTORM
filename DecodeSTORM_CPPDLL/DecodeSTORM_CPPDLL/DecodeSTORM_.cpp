...
(JNIEnv* env, jclass obj, jstring VoronoiSavePath, jint isLinkPostCluster)
{
	...
	if (isLinkPostCluster) {
		voronoi.VoronoiDiagram(RoiLoc1);
	}
	else {
		voronoi.VoronoiDiagram(RawRoiLoc1);
	}
}
...
(JNIEnv* env, jclass obj, jstring VoronoiSavePath, jint isLinkPostCluster) {
	...
	VoronoiResSave = jstring2str(env, VoronoiSavePath);
	if (isLinkPostCluster) {
		voronoi.VoronoiDiagram(RoiLoc2);
	}
	else {
		voronoi.VoronoiDiagram(RawRoiLoc2);
	}
}
...
/*
 * Class:     DecodeSTORM_
 * Method:    lm_GetVoronoiInfo
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_DecodeSTORM_1_lm_1GetVoronoiInfo
(JNIEnv* env, jclass obj) {
	return VoronoiClusterNum;
}
