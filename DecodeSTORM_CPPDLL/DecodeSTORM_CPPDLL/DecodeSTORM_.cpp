...
(JNIEnv* env, jclass obj)
{
	jdoubleArray result = env->NewDoubleArray(Doc1Size);

	env->SetDoubleArrayRegion(result, 0, Doc1Size, Doc1Array);
...
(JNIEnv* env, jclass obj)
{
	jdoubleArray result = env->NewDoubleArray(Doc2Size);

	env->SetDoubleArrayRegion(result, 0, Doc2Size, Doc2Array);
...
(JNIEnv* env, jclass obj)
{
	...
	float DocHistInfo[2];
	DocHistInfo[0] = Doc1Range;
	DocHistInfo[1] = Doc2Range;
	env->SetFloatArrayRegion(result, 0, 2, DocHistInfo);
...
(JNIEnv* env, jclass obj, jstring DocScoreResSave) {
	string DoC_Results_save = jstring2str(env, DocScoreResSave);
	//DoC_Results_save = DoC_Results_save + ".csv";
	field<std::string> header(DoC_Results.n_cols);
	header(0) = "Percentage of colocalised Ch1 molecules";
	header(1) = "Percentage of colocalised Ch2 molecules";
	DoC_Results.save(csv_name(DoC_Results_save, header));
}