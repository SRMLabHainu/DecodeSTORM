...
public class DecodeSTORM_ implements PlugInFilter {
    ...
    public native static void lm_SetSpatialStatisticsPara(float MaxAnaDis);
    ...
    //Filtering
    public native static void lm_Filtering(int isUnpFilter,  int isDupFilt, int isGrouping, int isDenFilt, int isFilterChan1, int isFilterChan2, int isSave);
    ...
    public native static int lm_GetVoronoiInfo();
    ...
    public class Scatter extends Thread {
        ...
        public void run() {
            ...
            if (MyConfigurator.DataPathChan1 != null && MyConfigurator.DataPathChan2 == null) {
                ...
                WindowW = Math.round(ScatImInfo[0] - ScatImInfo[1]);
                WindowH = Math.round(ScatImInfo[2] - ScatImInfo[3]);
                Dimension dim=Toolkit.getDefaultToolkit().getScreenSize(); 
                double ratio = 0;
                if(WindowW>dim.width &&WindowH<dim.height){
                    ratio = (double)dim.width/WindowW;
                    WindowH = (int)Math.round(WindowH*ratio);
                    WindowW = dim.width;
                }else if(WindowH>dim.height && WindowW<dim.width){
                    ratio = (double)dim.height/WindowH;
                    WindowW = (int)Math.round(WindowW*ratio);
                    WindowH = dim.height;
                }else if(WindowH>dim.height && WindowW>dim.width){
                    ratio = (double)dim.height/WindowH;
                    WindowW = (int)Math.round(WindowW*ratio);
                ...
                ScatterPlot.setWindowSize(WindowW, WindowH);
        ...
        public void run() {
            ...
            if (MyConfigurator.RenderCh1Enable == 1) {
                ...
                if (RecImgF.length > 10) {
                    ...
                    float[][] RecImgFMat = new float[RenderImInfo[0]][RenderImInfo[1]];
                    one2Two(RecImgF, RecImgFMat);
                    CleanIt clean = new CleanIt();
                    float[][] rowsCleaned = clean.cleanZeroRows(RecImgFMat);
                    float[][] transposed = clean.transpose(rowsCleaned);
                    float[][] colsCleaned = clean.cleanZeroRows(transposed);
                    float[][] RecImgFMatClean = clean.transpose(colsCleaned);                                      
                    
                    FloatProcessor Ch1ImageProcessor2D = new FloatProcessor(RecImgFMatClean);
//                    Ch1ImageProcessor2D.setPixels(RecImgF);
            ...
            if (MyConfigurator.RenderCh2Enable == 1) {
                ...
                if (RecImgF.length > 10) {
                    ...
                    FloatProcessor Ch2ImageProcessor2D = new FloatProcessor(RecImgFMatClean);
...
//            }
    ...
    public class Filtering extends Thread {
        ...
        public void run() {
            ...
            lm_Filtering(MyConfigurator.UnpFilterEnable, MyConfigurator.DupFiltEnable,MyConfigurator.MergeEnable, MyConfigurator.DenFiltEnable,
                 MyConfigurator.FilterChan1Enable, MyConfigurator.FilterChan2Enable, MyConfigurator.FilteringSave);
    ...
    public class SpatialStatistics extends Thread {
        ...
        public void run() {
            ...
            lm_SetSaveDir(MyConfigurator.ResultsFilePath);
            lm_SetSpatialStatisticsPara(MyConfigurator.MaxAnaDis);
            if (MyConfigurator.RdfEnable == 1) {
                ...
                curveFitter = new CurveFitter(XG[0], XG[1]);
                curveFitter.doFit(12);
                double[] fitPara = curveFitter.getParams();
                double ClusterSize = fitPara[3];
                String Str = String.format("%.5f", ClusterSize);
                double ClusterSize1 = Double.parseDouble(Str);
                String ClusterSizeStr =  Double.toString(ClusterSize1);
                String Lable = "Max G(r): " + GrBorderMax + " at Max r: " + MaxGrID + " Cluster size: " + ClusterSizeStr +"nm";
                SpatialStatisticsPlot.addLabel(0, 0, Lable);
            ...
            if (MyConfigurator.RipleyHEnable == 1) {
                ...
                double ClusterSize = RipleyImageInfo[2]/1.3;
                String Str = String.format("%.5f", ClusterSize);
                double ClusterSize1 = Double.parseDouble(Str);
                String ClusterSizeStr =  Double.toString(ClusterSize1);
                String Lable = "Max L(r)-r: " + HrBorderMax + " at Max r: " + MaxHrID + " Cluster size: " + ClusterSizeStr +"nm";
                SpatialStatisticsPlot.addLabel(0, 0, Lable);
        ...
        public void run() {
            ...
            if (MyConfigurator.DBSCANEnable == 1) {
                if (MyConfigurator.ClusterChan1Enable == 1) {
                    ...
                    String DensityStr = String.format("%.3f",DbscanImCh1Info[3] / ((roiInfo.Width_nm * roiInfo.Height_nm) / 1000000)); 
                    String DbscanCh1Info = "Channel_1 DBSCAN: " + ClusterNum + " clusters were identified. Density:" + DensityStr + "/μm²" ;
                ...
                if (MyConfigurator.ClusterChan2Enable == 1) {
                    ...
                    String DensityStr = String.format("%.3f",DbscanImCh2Info[3] / ((roiInfo.Width_nm * roiInfo.Height_nm) / 1000000));
                    String DbscanCh2Info = "Channel_2 DBSCAN: " + ClusterNum + " clusters were identified. Density:" + DensityStr + "/μm²" ;
                    UIManager.put("OptionPane.okButtonText", "Ok");
            ...
            if (MyConfigurator.VoronoiEnable == 1) {
                if (MyConfigurator.ClusterChan1Enable == 1) {
                    ...
                    int VoronoiClusterNum = lm_GetVoronoiInfo();
                    String VoronoiClusterNumStr  = Integer.toString(VoronoiClusterNum);
                    String VoronoiClusterDensity  = String.format("%.3f",VoronoiClusterNum / ((roiInfo.Width_nm * roiInfo.Height_nm) / 1000000));                          
                    String VoronoiCh1Info = "Channel_1: " + VoronoiClusterNumStr + " clusters were identified. Density:" + VoronoiClusterDensity + "/μm²" ;
                    UIManager.put("OptionPane.okButtonText", "Ok");
                    JOptionPane.showMessageDialog(null, VoronoiCh1Info, "Channel_1 VoronoiInfo", JOptionPane.PLAIN_MESSAGE);
                }
                if (MyConfigurator.ClusterChan2Enable == 1) {
                    if (MyConfigurator.SegmentationClusterSave == 1) {
                        String SegmentationAndClusterSaveDir = MyConfigurator.ResultsFilePath + "SegmentationAndClusterResults";
                    ...
                    String VoronoiClusterNumStr  = Integer.toString(VoronoiClusterNum);
                    String VoronoiClusterDensity = String.format("%.3f",VoronoiClusterNum / ((roiInfo.Width_nm * roiInfo.Height_nm) / 1000000));                    
                    String VoronoiCh2Info = "Channel_2: " + VoronoiClusterNumStr + " clusters were identified. Density:" + VoronoiClusterDensity + "/μm²" ;
                    UIManager.put("OptionPane.okButtonText", "Ok");
                    JOptionPane.showMessageDialog(null, VoronoiCh2Info, "Channel_2 VoronoiInfo", JOptionPane.PLAIN_MESSAGE);
                }
    ...
    public float[][] cleanZeroRows(float[][] matrix) {
        ...
        if (count == 0){
            ...
            float[][] newMatrix = new float[rows - count][cols];
            int idx = -1;
            for(int i=0;i<rows;i++){
                if (!deleteThese[i]){
                    idx++;
