...
public class DecodeSTORM_ implements PlugInFilter {
    ...
    public native static void lm_SetColocalizationPara(float Radius, float Rmax, float Step, float ColoThres, double width, double height);
    ...
    public native static double[] lm_GetDoc1Hist();

    public native static double[] lm_GetDoc2Hist();

    public native static float[] lm_GetDocHistInfo();


    public native static void lm_SaveColoclizationResult(String DocScoreResSave);
    ...
    public class Colocalization extends Thread {
        ...
        public void run() {
            ...
            lm_SetColocalizationPara(MyConfigurator.Radius, MyConfigurator.Rmax, MyConfigurator.Step, MyConfigurator.ColoThres, roiInfo.Width_nm, roiInfo.Height_nm);

            lm_Colocalization(MyConfigurator.LinkPostColocEnable);

            float[] DocHistInfo = lm_GetDocHistInfo();
       
            Plot ColocPlot;
            FileSaver ColocalizationTifSaver;

            double[] Doc1 = lm_GetDoc1Hist();
            ColocPlot = new Plot("DOC Score Ch1 -> Ch2", "DOC Score Ch1 -> Ch2", "Frequency");
            ColocPlot.setColor(Color.green);
            ColocPlot.addHistogram(Doc1, DocHistInfo[0] / 100);
            ColocPlot.show();
            ImagePlus Doc1ImPlus = ColocPlot.getImagePlus();
            double[] Doc2 = lm_GetDoc2Hist();
            ColocPlot = new Plot("DOC Score Ch2 -> Ch1", "DOC Score Ch2 -> Ch1", "Frequency");
            ColocPlot.setColor(Color.red);
            ColocPlot.addHistogram(Doc2, DocHistInfo[1] / 100);
            ColocPlot.show();
            ImagePlus Doc2ImPlus = ColocPlot.getImagePlus();
            ...
            if (MyConfigurator.ColocalizationSave == 1) {
                ...
                lm_SaveColoclizationResult(ColocalizationSaveDir + "\\DocScoreResults.csv");
                ColocalizationTifSaver = new FileSaver(Doc1ImPlus);
                ColocalizationTifSaver.saveAsTiff(ColocalizationSaveDir + "\\Channel_1-Channel_2_DocScore.tif");
                ColocalizationTifSaver = new FileSaver(Doc2ImPlus);
                ColocalizationTifSaver.saveAsTiff(ColocalizationSaveDir + "\\Channel_2-Channel_1_DocScore.tif");
            }
            MyConfigurator.EnableColocalization();
        }
    }
